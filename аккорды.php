...
<body>
	<div id="glaw">
		...
		<div class="blok2">
			...
			<button class="button_note" name="button_note1"  onclick="window.location.href = 'аккордыБД1.php'"> 
				<?php 
			...
			<button class="button_note" name="button_note2"  onclick="window.location.href = 'аккордыБД2.php'"> 
				<?php 
				  $dbc = mysqli_connect('localhost', 'root' ,'', 'site');
				  $query = "SELECT COUNT(*) FROM `chords`";
				  $count = mysqli_query($dbc, $query);
			...
			<button class="button_note" name="button_note3"  onclick="window.location.href = 'аккордыБД3.php'"> 
				<?php 
				  $dbc = mysqli_connect('localhost', 'root' ,'', 'site');
				  $query = "SELECT COUNT(*) FROM `chords`";
				  $count = mysqli_query($dbc, $query);
				  $c = mysqli_fetch_array($count);	
				  $query1 = "SELECT * FROM `chords` WHERE chords_id = $c[0]-2";
				  $data = mysqli_query($dbc, $query1);
				  $row = mysqli_fetch_row($data);
				  echo "<h3> $row[1] - $row[2]</h3>";	
				?>
